...
## 1. IADDQ
##    IADDQ needs: ValC, REG
##    IADDQ can use forwarding
##    IADDQ may cause load-use hazard
## 2. Forward-Plus
##    Load and Write doesnt actually change the value of register, so we can
##      forward ValM to e-ValE to avoid a bubble. Because Mem Phase is longer
##      than Exec Phase, such forwarding will not slow the pip cycle in general
####################################################################
...
# Lets forward
word e_valA = [
  (M_icode == IMRMOVQ && E_icode == IRMMOVQ) && (E_srcA == M_dstM) : m_valM;
  1 : E_valA;
];
