...
//int Rio_readnEH(int fd, void *ptr, size_t nbytes) {
//  return rio_readn(fd, ptr, nbytes) >= 0;
//}
...
int Rio_writenEH(int fd, void *usrbuf, size_t n) {
  return rio_writen(fd, usrbuf, n) == n;
}
...
long Rio_readnbEH(rio_t *rp, void *usrbuf, size_t n) {
  return rio_readnb(rp, usrbuf, n);
}
...
ssize_t Rio_readlinebEH(rio_t *rp, void *usrbuf, size_t maxlen) {
...
int writeEH(int fd, void *buffer, size_t n) {
  size_t n_left = n;
  ssize_t n_written;
...
int readEH(int fd, void *buffer, size_t n) {
  size_t n_left = n;
  ssize_t n_read;
  struct pollfd poll_fd;
  poll_fd.fd = fd;
...
size_t readlineEH(int fd, void *buffer, size_t n, char terminate) {
  for (int i = 0; i < n; ++i) {
    if (readEH(fd, buffer + i, 1) != 1) {
      return i + 1;
    }
    if (((char *) buffer)[i] == terminate) {
      return i + 1;
    }
  }
  return n;
